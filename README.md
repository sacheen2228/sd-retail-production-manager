...
- **Purchase Orders** — create a full order in one flow: register a new retailer/brand partner on the spot, add multiple style lines (style code, category, sub-category, **color, size**, qty, price, fabric, trims, stage), auto-computed order total. Expand any PO to see its styles.
...
- **Stock Report** — **ready-stock inventory of finished garments** in a row-wise ledger: **Sr, SKU Code, Item Name, Category, Color, Size, Warehouse, Opening, Received, Issued, Closing, Min, Cost, Value, Status**. Issued units come from dispatched deliveries, Closing is current on hand, and Opening is derived from the ledger identity (Opening = Closing + Issued − Received); **+ Stock** records Received. Filter by SKU, color, size or out-of-stock-only, **group by SKU** for a size-wise variant breakdown, **barcode/SKU scan** jumps straight to an item, Style Code is required and duplicate variants are rejected. CSV / Google Sheets export. (Fabric & trim allocation is available under **Partners & Stock → Fabric Stock Report**.)
- **Reports** — row-wise **WIP Report**: **Sr, PO, Buyer, Style, Color, Size, Order Qty, WIP, Stage, Days, Dispatch, Status**, plus delivery risk list, by-stage and by-retailer summaries, **CSV export** and **→ Google Sheet export** (see `docs/SHEET_EXPORT.md`).
