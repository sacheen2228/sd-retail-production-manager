...
app.get('/api/reports/merchandising', (_req, res) => {
  const styles = db.styles || []
  ...
  fabrics.forEach((f) => {
    ...
    if (!v) return
    const g = (vendorMap[v] = vendorMap[v] || { name: v, items: 0, stockValue: 0, leadSum: 0, lowItems: 0, reorderItems: 0 })
    g.items++
    g.stockValue += (Number(f.stock) || 0) * (Number(f.costPrice) || 0)
    g.leadSum += Number(f.leadTimeDays) || 0
    if ((Number(f.stock) || 0) <= (Number(f.lowStockLevel) || 30)) g.lowItems++
    const cons = Number(f.consumption) || 0
    ...
    if ((Number(f.stock) || 0) - allocated < 0) g.reorderItems++
  })
  ...
  res.json({
    summary: {
      sellThroughPct: totalSold + totalOnHand > 0 ? (totalSold / (totalSold + totalOnHand)) * 100 : 0,
      totalSold,
      totalOnHand,
      gmroi: inventoryValue > 0 ? grossProfit / inventoryValue : 0,
      inventoryTurn: inventoryValue > 0 ? cogs / inventoryValue : 0,
      inventoryValue,
      grossProfit,
      avgProductionDays,
      bestSellerCount: codeRows.filter((r) => r.sold > 0).length
    },
    sellThroughRows: codeRows.sort((a, b) => b.total - a.total),
    bestSellers: codeRows.filter((r) => r.sold > 0).sort((a, b) => b.sold - a.sold).slice(0, 5),
    slowSellers: codeRows.filter((r) => r.onHand > 0 && r.sold === 0).sort((a, b) => b.onHand - a.onHand).slice(0, 5),
    fabricConsumption,
    vendorPerformance,
    stockAging: aging,
    topOrders: db.purchaseOrders
      .slice()
      .sort((a, b) => (Number(b.value) || 0) - (Number(a.value) || 0))
      .slice(0, 5)
      .map((o) => ({ poNumber: o.poNumber, retailer: rName(o.retailerId), value: Number(o.value) || 0, status: o.status, deliveryDate: o.deliveryDate }))
  })
})
